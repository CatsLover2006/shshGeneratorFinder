...
bool tihmstar::img4tool::isGeneratorValidForIM4M(const ASN1DERElement &im4m, uint64_t generator) noexcept{
    try {
        ...
        if (bnch.payloadSize() == SHA_DIGEST_LENGTH) {
            std::array<char, SHA_DIGEST_LENGTH> tmp{'\0'};
            std::string hash{tmp.begin(),tmp.end()};
            SHA1((unsigned char*)&generator, sizeof(generator), (unsigned char *)hash.c_str());
            return memcmp(hash.c_str(), bnch.payload(), bnch.payloadSize()) == 0;
        }else{
            std::array<char, SHA384_DIGEST_LENGTH> tmp{'\0'};
            std::string hash{tmp.begin(),tmp.end()};
            SHA384((unsigned char*)&generator, sizeof(generator), (unsigned char *)hash.c_str());
            return memcmp(hash.c_str(), bnch.payload(), bnch.payloadSize()) == 0;
        }
    } catch (...) {
        return false;
